...
> **Bônus do kit.** Esta skill é um presente para quem quer usar uma página existente como referência. Não faz parte do caminho principal da aula: a rota padrão é a skill `pagina` (opção "a partir de uma referência"), que não precisa de navegador automatizado.
>
> Para usar esta skill você precisa da extensão **Claude in Chrome** e de **Python 3**. Ela gera três coisas: `deploy-ready/` (cópia fiel, que **mantém scripts e rastreadores do site original e não deve ser publicada como sua**), `copy.md` (estrutura e textos, só para referência) e `clean/` (HTML novo e limpo). **Use apenas `clean/` como base**, troque fotos, nomes, depoimentos e contatos pelos seus, e confira com `scripts/verificar_pagina.py` do kit antes de qualquer publicação. Leia `rules/copyright.md`.
...
**Regra crítica (aprendido na dor):** Na primeira execução (a página de teste), reescrevi o clean sem extrair design system antes → ficou "muito fora da realidade" (identidade visual divergente). A extração prévia força a reescrita a ser **tradução** e não criação. Ver `rules/design-system.md` pros detalhes.
...
**Regra crítica (aprendido na dor):** Na segunda execução da skill (outra página Posicione-se, 2026-04) quebrei 6 dessas checagens simultaneamente — hero centralizou, logo sumiu, grid virou card, fotos de modelos sumiram, thumbnail no lugar de paisagem, inline no lugar de modal. Hoberdan pediu reescrita completa.
...
**Usuário:** "Baixa `https://exemplo.com/pagina-de-vendas/` como cliente Exemplo, nicho psicologia, tipo ingresso-pago, slug imersao-ecr01"
...
(O exemplo completo de um caso real não é distribuído no kit.)
...
Esta skill foi inicialmente calibrada com a página da a página de teste (WordPress + Elementor Pro + OMGF + PixelYourSite). Cada nova página processada deve:
